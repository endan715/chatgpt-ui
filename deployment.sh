...
read -p "Please set a port for the frontend server [default: 8080]: " CLIENT_PORT
...
sudo APP_DOMAIN="${APP_DOMAIN}:${SERVER_PORT},${APP_DOMAIN}" CLIENT_PORT=${CLIENT_PORT} SERVER_PORT=${SERVER_PORT} WSGI_PORT=${WSGI_PORT} DB_URL=${DATABASE_URL}  $DOCKER_COMPOSE_CMD up -d
